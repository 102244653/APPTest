...
public class ExcelUtil {
    ...
    public ExcelUtil(String path, String sheetname){
        try {
            ...
            TestLog.info("ExcelUtil: ExcelUtil()","读取到"+sheetname);
    ...
    public String readFromExcelCell(int rowIndex, int cellIndex){
        ...
        if (row == null) {
            TestLog.error("ExcelUtil: readFromExcelCell","第"+rowIndex+"行不存在");
            return  null;
        }
        HSSFCell cell = row.getCell(cellIndex);
        if (cell == null) {
            TestLog.error("ExcelUtil: readFromExcelCell","第"+cellIndex+"列不存在");
            return  null;
        }
        ...
        switch (cellType)
        {
            ...
            case 1 :
            {
                ...
                TestLog.info("ExcelUtil: readFromExcelCell","读取到单元格["+rowIndex+"."+cellIndex+"]的值："+S);
                break;
            }
            case 0 :
            {
    ...
    public String[] readFromExcelrow(int rowIndex) {
        HSSFRow row = sheet.getRow(rowIndex);
        if (row == null) {
            TestLog.error("ExcelUtil: readFromExcelrow","第"+rowIndex+"行不存在");
            return  null;
        }
        ...
        for(int i=0;i<row.getLastCellNum();i++){
            ...
            switch (cellType)
            {
                ...
                case 1 :
                {
                    ...
                    TestLog.info("ExcelUtil: readFromExcelrow","读取到单元格["+rowIndex+"."+i+"]信息："+result[i]);
                    break;
                }
                case 0 :
                {
    ...
    public String[] readFromExcelcolumn(int columnIndex){
        ...
        for (int k=0;k<rowid;k++){
            ...
            if (row == null) {
                TestLog.error("ExcelUtil: readFromExcelcolumn","第"+k+"行不存在");
                return  null;
            }
            HSSFCell cell = row.getCell(columnIndex);
            if (cell == null) {
                TestLog.info("ExcelUtil: readFromExcelcolumn","第"+columnIndex+"列不存在");
                return  null;
            }
            int cellType = cell.getCellType();
            switch (cellType)
            {
                ...
                case 1 :
                {
                    ...
                    TestLog.info("ExcelUtil: readFromExcelCell","读取到单元格["+columnIndex+"."+k+"]信息："+result[k]);
                    break;
                }
                case 0 :
                {
    ...
    public void writeToExcelCell(int rowIndex, int cellIndex, String value) {
        ...
        if (row == null) {
            ...
            TestLog.error("ExcelUtil: writeToExcelCell","行号不存在，新建行"+rowIndex);
        }
        HSSFCell cell = row.getCell(cellIndex);
        //判断列是否存在
        if (cell == null) {
            cell = row.createCell(cellIndex);
            TestLog.info("ExcelUtil: writeToExcelCell","单元格不存在，新建单元格["+rowIndex+"."+cellIndex+"]");
        }
        cell.setCellValue(value);
        TestLog.info("ExcelUtil: writeToExcelCell","单元格["+rowIndex+"."+cellIndex+"]成功写入信息："+value);
    }
    ...
    public int getNullRow() {
        ...
        for (int i=1;;i++) {
            ...
            if (result == null) {
                nullrow=i;
                TestLog.info("ExcelUtil: getNullRow","第"+i+"行的值为空");
                break;
            }
        }
        return nullrow;
    ...
    public int getrows(){
        ...
        TestLog.info("ExcelUtil: getrows","当前页总行数为："+(row+1));
        return row;
    }
    ...
    public void writeToExcelRows(String[] value,int h) {
        ...
        for(int i=0;i<value.length;i++){
            ...
            TestLog.info("ExcelUtil: writeToExcelRows","单元格["+rowid+"."+i+"]写入值："+value[i]);
        }
    }
    ...
    public void save (String path){
        try {
            ...
            TestLog.info("ExcelUtil: save",path+"保存成功");
        }
        catch (Exception e){TestLog.info("ExcelUtil: save",path+"保存失败");}
    }

    //创建一个Excel
    public void CreatExcel(String path,String[] value){
        try {
            // 创建 一个excel文档对象
            ...
            row.setHeightInPoints(30);// 设置行高23像素
            ...
            TestLog.error("ExcelUtil: CreatExcel","生成EXCEL文件失败");
        }
    }
